...
let CustomValidationPipe = class CustomValidationPipe extends common_1.ValidationPipe {
    ...
    handleValidationErrors(errors) {
        ...
        if (firstError.constraints) {
            throw new common_1.BadRequestException(firstError.constraints[Object.keys(firstError.constraints)[0]]);
        }
        else if (firstError.children.length > 0) {
            this.findError(firstError.children);
        }
    }
    findError(currentNode) {
        const node = currentNode[0];
        if (node?.constraints) {
            throw new common_1.BadRequestException(node.constraints[Object.keys(node.constraints)[0]]);
        }
